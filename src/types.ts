import type { $Fetch, FetchError, FetchOptions as Options } from 'ohmyfetch'
...
export interface OFetchInstance {
    /**
    ...
    /**
     * Fetches the `url` with the option `{method: 'options'}`.
     *
     * @param url - `Request` object, `URL` object, or URL string.
     * @returns Promise with `Body` method added.
     */
    options(url: Request | URL | string, options?: Options): Promise<any>;
     ...
     * @returns Promise that resolves to JSON parsed value.
     */
    $options<T = unknown>(url: Request | URL | string, options?: Options): Promise<T>;
     ...
     */
    onRequestError(callback: OnErrorHook): void
     ...
     * This hook enables you to globally handle request errors.
     */
    onResponseError(callback: OnErrorHook): void
